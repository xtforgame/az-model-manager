...
export default class AsuOrm {
  constructor(sequelizeDb, asuModelDefs) {
    ...
    Object.keys(associationModels).map(name => (this.associationModelInfo[name] = new AssociationModel(this, name, associationModels[name])));

    Object.keys(models).map(name => (this.tableInfo[name] = new DataModel(this, name, models[name])));

    Object.keys(this.tableInfo).map(name => this.tableInfo[name].setupAssociations());
  }
